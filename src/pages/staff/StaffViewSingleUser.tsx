    ...
    const saveDisableReason = async () => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                toast.error(response.message || "An error occurred while disabling the user.");
            }
        } catch (error) {
        ...
        <div className="p-6 bg-gray-50 min-h-screen">
            <div className="bg-white shadow-md rounded-lg max-w-5xl mx-auto p-6">
                ...
                <div className="mb-6 p-4 bg-gray-200 rounded-lg">
                    <h2 className="text-lg font-semibold mb-2 text-gray-800">Account Status</h2>
                    <p className={`font-medium ${user.isDisabled ? "text-red-600" : "text-green-600"}`}>
                        {user.isDisabled ? "Disabled" : "Active"}
                    </p>
                    <button
                        className={`mt-4 px-4 py-2 rounded-md ${user.isDisabled ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"
                            } text-white`}
                        onClick={!user.isDisabled ? () => setShowModal(true) : () => handleEnableUser(user._id)}
                    >
                        {isFormLoading
                            ? "Loading..."
                            : user.isDisabled
                ...
                {isEditing ? (
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <p className="text-gray-700">{user.rank || "No rank assigned"}</p>
                        </div>
                        <div>
                ...
                {showModal && (
                    <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50">
                        <div className="bg-white p-6 rounded-md w-96">
                            <h3 className="text-xl font-semibold">Reason for Disabling User</h3>
                            <textarea
