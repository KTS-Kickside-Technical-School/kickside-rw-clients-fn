...
const Homepage = () => {
    ...
    const renderArticle = (article: any) => (
        <div className="relative h-[450px] sm:h-[300px] md:h-[400px] lg:h-[200px] rounded-lg overflow-hidden" key={article?._id}>
            <img
        ...
        <ol className="space-y-3">
            {articles.slice(4, 8).map((article: any, index) => (
                <li className="text-white text-sm md:text-lg font-medium flex" key={article?._id}>
                    ...
                    <Link to={`/news/${article?.slug}`} className="hover:text-gray-200 transition">
                        {article?.title}
                    </Link>
                </li>
            ))}
                        ...
                        {loading ? (
                            ...
                            <>
                                ...
                                <div className="flex-1 lg:flex-grow px-4">
                                    <h1 className="font-bold text-white text-lg md:text-2xl mb-4">Top Headlines</h1>
                                    {renderTopHeadlines()}
                                </div>
                            </>
                        )}
