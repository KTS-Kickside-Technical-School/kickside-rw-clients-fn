...
const ArticleDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSingleArticle = async (slug: string) => {
      ...
      try {
        ...
        if (response.data.article.language === 'kinyarwanda')
          navigate(`/news/${response.data.article.slug}`);
  ...
  if (error) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center bg-gray-50 px-4 py-8 text-center">
